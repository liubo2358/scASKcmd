function [SLC_Model_FIT,FIT_accuracy] = sliceprerws(in_X_new,true_labs_new,binary_mod,class_num,slice_vle,SLC_Model_RWS)
...
true_labs_Fit = num2cell(zeros(2,length(slice_vle)));
for m = 1:size(SLC_Model_RWS,1)
    for n = 1:length(slice_vle)
        true_labs_fit = predict(SLC_Model_RWS{m,n,1},in_X_new_slc(:,:,n));
        true_labs_Fit{m,n} = numclass2mat(true_labs_fit,class_num,'row');
    end
end
...
for m = 1:size(SLC_Model_RWS,1)
    for n = 1:length(slice_vle)
        true_labs_FIT = true_labs_FIT + SLC_Model_RWS{m,n,2}.*true_labs_Fit{m,n};
    end
end
...
if strcmp(accuracy_tag,'on')
    ...
    accuracy_str = ['% the Accuracy of SLC_Model_RWS on Testing Dataset is: ',num2str(true_labs_FIT_Accuracy*100),'%'];
    ...
    title('Confusion Matix of SLC\_Model\_RWS'); retouchcfm(f1,f1a1);
    savefig(f1,[cd,'\figure\figure61_sliceprerws.fig']);
    saveas(f1,[cd,'\figure\figure61_sliceprerws.bmp']);
end
